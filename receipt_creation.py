...
BOLD_LOCATIONS = [
    ((83, 36), 20, (0, 0, 0, 255), "name", "lb"),  # Name #1
...
NORMAL_LOCATIONS = [
    ((982, 436), 25, (0, 0, 0, 255), "amount", "rb"),
...
FONT_TYPES = [
    ("paypal_fonts/PayPalSansBig-Regular.woff", BOLD_LOCATIONS),
...
def image_creation(amount, name):
    ...
    while file_size < 100:
        with Image.open(DEFAULT_IMAGE_PATH).convert("RGBA") as img:
            ...
            for font_type, locations in FONT_TYPES:
                for values in locations:
                    font = ImageFont.truetype(font_type, values[1])
                    value = assess_value(values, amount, name)
                    if values[2] == (16, 114, 235, 255):
                        value += " schreiben"
                    elif values[0] == (984, 38):
                        value = f"- {value}"
                    d.text(
                        values[0], value, font=font, fill=values[2], anchor=values[4]
                    )
...
def assess_value(values, amount, name):
    if "name" in values:
        return name
